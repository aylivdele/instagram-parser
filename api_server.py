...
def remove_competitor(username):
    """Удалить конкурента"""
    try:
        # Здесь нужно добавить метод удаления в InstagramMonitor
        return jsonify({
            "success": True,
            "message": f"Конкурент @{username} удален"
        })
    except Exception as e:
...
def trigger_manual_check():
    """Запустить внеплановую проверку"""
    try:
        # Запускаем проверку в фоне
        async def check():
            await monitor.monitor_cycle("YOUR_CHAT_ID")
    ...
    try:
        ...
        return jsonify({
            ...
            "data": {
                ...
                "last_check": "just now"  # TODO: добавить реальное время
            }
        })
    except Exception as e:
